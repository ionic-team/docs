...
function preset(context, opts) {
    if (opts === void 0) { opts = {}; }
    var siteConfig = context.siteConfig, siteDir = context.siteDir;
    ...
    themes.push(function () {
        var hasAddedCascadeLayersDeclaration = false;
        return {
            ...
            getClientModules: function () {
                return [
                    path_1["default"].join(__dirname, "../src/styles/custom.scss"),
            ...
            configurePostCss: function (postCssOptions) {
                var plugin = {
                    ...
                    prepare: function (result) {
                        var isBaseStyle = result.opts.from.includes("@docusaurus");
                        var isDsStyle = result.opts.from.includes("@ionic-internal/design-system") ||
                            result.opts.from.includes("infima");
                        var isPresetStyle = result.opts.from.includes("@ionic-internal/preset-classic") ||
                            result.opts.from.includes("Ionic/preset-classic");
                        var isLocalStyle = result.opts.from.includes(siteDir);
                        var param = isBaseStyle
                            ? "base"
                            : isDsStyle
                                ? "ds"
                                : isPresetStyle
                                    ...
                                    : isLocalStyle
                                        ...
                                        : null;
                        if (!param) {
                            throw new Error("Unable to determine layer for ".concat(result.opts.from));
                        }
                        return {
                            Once: function (root, _a) {
                                var AtRule = _a.AtRule;
                                var layer = new AtRule({ name: "layer", params: param });
